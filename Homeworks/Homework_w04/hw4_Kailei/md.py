...
def f_coulomb(pos, charge):
    ...
    for idx in range(distances.shape[0]):
        ...
        f[idx] = [np.sum(x * (F_scalar)), np.sum(y * (F_scalar)), np.sum(z * (F_scalar))]
...
def f_lennard_jones(positions, sigmas, epsilons):
    ...
    for idx in range(distances.shape[0]):
        sigma = (sigmas+sigmas[idx])/2
        du_dr = (
            4 * epsilons
            * (
                -12 * ((distances[idx])**(-13)/ sigma**(-12)) 
                + 6 * ((distances[idx])**(-7)/ sigma**(-6))           
            )
        )
        # get each component of the distance
