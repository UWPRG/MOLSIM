...
def f_coulomb(pos, charge):
    """
    Calculate interatomic forces by coulombic interactions.
    ...
    -------
    f : array-like
        Coulombic forces experiences by each atom in the MD frame
    """
    # Coulomb's constant, which may need to have units fixed...
    ke = 9*10**9*(1/6.242**18)**2*(1/10**-9)**2 # N nm^2 / (elem. charge)^2
    ## check this and do some checking/dim analysis on the LJ potential

    # prepare pair-wise distance matrix
    distances = distance_matrix(pos, pos)
    np.fill_diagonal(distances, 100000)

    # prepare pair-wise charge product matrix
    charge_prod = np.outer(charge, charge)
    np.fill_diagonal(charge_prod, 0.0001)

    # blank array for forces
    f = np.zeros_like(pos)

    # calculate all the different force components
    for idx in range(1,distances.shape[0]):
        # calculate coulomb law force
        du_dr = (-ke * charge_prod[idx] / distances[idx]**2 )
...
def f_lennard_jones(positions, sigmas, epsilons):
    ...
    for idx in range(1, distances.shape[0]):
        du_dr = (
            ...
            * epsilons
            * (
                12 * (sigmas**12)/(distances[idx]**13)
                - 6 * (sigmas**6)/(distances[idx]**7)
            )
        )
        # get each component of the distance
