...
def f_coulomb(pos, charge):
    ...
    # Reference: http://manual.gromacs.org/2019/reference-manual/definitions.html#defunits
    # F=(1/4*pi*epsilon)q**2/r**2
    # electric conversion facror (1/4*pi*epsilon)
    k = 138.935458 #units: kJ.mol**-1.nm.e**-2
    # calculate forces
    f = np.zeros_like(pos)
    for idx in range(distances.shape[0]):
        charge_list = []
        for i in range(len(charge)):
            charge_list.append(charge[idx]*charge[i])
        charge_array=np.array(charge_list)
        F = - k*charge_array / distances[idx]**2
        ...
        f[idx] = [np.sum(x * F), np.sum(y * F), np.sum(z * F)]
    return f
